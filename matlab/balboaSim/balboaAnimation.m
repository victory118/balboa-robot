classdef balboaAnimation
    ...
    methods
        %------constructor-----------
        function self = balboaAnimation(P)
            self.ell = P.ell;
            self.width = P.w;
            self.height = P.h;
            self.gap = P.gap;
        ...
        function self=draw_bob(self, z, theta)
            th = 0:2*pi/10:2*pi;
            % center of bob
            Xc = z + self.ell*sin(theta);
            Yc = self.gap + self.height + self.ell*cos(theta);
            % points that define the bob
            X = Xc + self.radius*cos(th);
            Y = Yc + self.radius*sin(th);
            % plot or update bob
